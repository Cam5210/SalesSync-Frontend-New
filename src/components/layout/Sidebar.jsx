  ...
  return (
    <aside className={`sidebar bg-gray-100 dark:bg-gray-800 shadow-md transition-all duration-300 ${
      ...
      <div className={`h-full flex flex-col ${isOpen ? 'p-4' : 'p-0 md:py-4 md:px-2'}`}>
        ...
        {/* Render custom sidebar content passed as children if available */}
        {children ? (
          <div className={`flex-1 overflow-y-auto ${!isOpen && 'hidden md:block'}`}>
            {isOpen ? (
              children
            ) : (
              // For collapsed state, we'll show the icon-only view of the children
              // This assumes the children have a collapsedView property/component
              <div className="flex flex-col items-center space-y-4">
                {React.Children.map(children, child => 
                  React.isValidElement(child) && child.props.collapsedIcon
                    ? React.cloneElement(child.props.collapsedIcon, {})
                    : null
                )}
              </div>
            )}
          ...
          // Default sidebar content when no children are provided
          <>
            {isOpen ? (
